...
window.onload = function () {
	...
	w2utils.locale(localStorage.getItem("locale") || w2utils.settings.locale).then(function (evt) {
		...
		divLang.z(["select", {
			...
			onchange: function () {
				localStorage.setItem("locale", this.value); location.reload();
			}
		}]);
		divLang.z(["select", {
			innerHTML: "<option value='w2ui.min'>🩶</option><option value='mi-blue'>🩵</option><option value='mi-dark-blue'>💙</option><option value='sp'>🤍</option><option value='sp-dark'>🖤</option><option value='res'>❤️</option>",
			value:n$.theme,
			onchange: function () {
				n$.theme = this.value ||"w2ui.min"; localStorage.setItem("theme", n$.theme); cssMain.href="lib/"+n$.theme+".css";
			}
		}]);
	});
}
...
function openDesktop(force) {
	(w2ui["layMain"] || new w2layout({
		...
		panels: [
			...
			{ type: 'bottom', size: "40%", resizable: true, html: '<div id="divPreview" class="nut-full"></div>', hidden: true }
		],
	})).render(divLogin);

	NUT.ds.select({ url: NUT.URL + "nv_wfflow", orderby: "flowid desc", where: [["toroleid", "=", n$.user.roleid], ["touserid", "in", [0, n$.user.userid]], ["isread", "=", 0]] }, function (res) {
		if (res.success) {
			var items = [];
			var maxprior = 0;
		...
		NUT.ds.get({ url: NUT.URL_TOKEN + "app/" + id }, function (res) {
			...
			if (res.success) {
				...
				for (var i = 0; i < result.menus.length; i++) {
					...
					if(menu.maplayer){
						node.id=menu.maplayer;
						node.maplayer=menu.maplayer;
						node.collapsible=node.plus=true;
						if(menu.subtype)node.nodes=[{id:"sub_"+menu.menuid,text:"<i>"+menu.subtype+"</i>"}];
					}
				...
				var opt={
					...
					flat: NUT.isMobile,
					nodes: nodes,
					topHTML:"<input id='_txttopsearch' class='w2ui-input' placeholder=" + NUT.w2utils.lang("_Search") + " style='width:100%'/>",
					onToggle:function(){alert(1)},
				...
				/*if (n$.app.startexec) {
					setTimeout(function(){
						var a = NUT.createWindowTitle(n$.app.startexec, divTitle);
						if (a) {
							NUT.runComponent(n$.app.startexec, a.div);
						}
					},1000);
				}*/
		...
		if (NUT.isGIS) {
			NUT.ds.select({ url: NUT.URL + "nv_appservice_service", orderby: "seqno", where: [["appid", "=", id], ["servicetype", "=", "arcgis"]] }, function (res) {
				if (res.success && res.result.length) {
					divMain.innerHTML = "<div id='divMap' class='nut-full'></div><div id='tbrMap' style='position:absolute;top:10px;right:10px'></div><button style='position:absolute;border:none;top:0px;right:0px' onclick='NUT.w2ui.layMain.toggle(NUT.isMobile?\"bottom\":\"right\")'>&nbsp;»&nbsp;</button>";
			...
			if (NUT.isGIS && NUT.isMobile) {
				...
				divPreview.innerHTML = titleHtml;
