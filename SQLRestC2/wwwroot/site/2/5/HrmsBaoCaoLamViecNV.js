var HrmsBaoCaoLamViecNV={
	run:function(p){
		...
		NUT.ds.select({url:HrmsBaoCaoLamViecNV.url+"data/chamcong_v",select:"sum(cong)cong,sum(keg)keg,sum(bold)bold,sum(boldl)boldl,sum(light)light,sum(lightl)lightl,sum(trucbachl)trucbachl,sum(hanoipre)hanoipre,sum(hanoiprel)hanoiprel",where:[["madoitac","=","HABECO"],["nam","=",nam],["thang","=",thang],["manhanvien","=",n$.user.username]]},function(res){
